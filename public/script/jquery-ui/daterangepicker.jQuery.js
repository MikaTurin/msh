function trans(ru, lv, en) {
  if (typeof(lng) != 'string') return en;
  else if (lng == 'ru') return ru;
  else if (lng == 'lv') return lv;
  return en;
}
/**
 * --------------------------------------------------------------------
...
jQuery.fn.daterangepicker = function(settings){
    ...
    presetRanges: [
      {text: trans('Сегодня', 'Šodien', 'Today'), dateStart: 'today', dateEnd: 'today' },
      {text: trans('Вчера', 'Vakar', 'Yesterday'), dateStart: 'yesterday', dateEnd: 'yesterday' },
      {text: trans('Последние 7 дней', 'Pēdējās 7 dienās', 'Last 7 days'), dateStart: 'today-7days', dateEnd: 'today' }, /*Last 7 days*/
      {text: trans('Последние 30 дней', 'Pēdējās 30 dienās', 'Last 30 days'), dateStart: 'today-30days', dateEnd: 'today' }, /*Last 30 days*/
      {text: trans('С начала месяца', 'Kopš mēneša sākuma', 'Month to date'), dateStart: function(){ return Date.parse('today').moveToFirstDayOfMonth();  }, dateEnd: 'today' },  /*Month to date*/
      {text: trans('Предыдущий месяц', 'Iepriekšējais mēnesis', 'Previous month'), dateStart: function(){ return Date.parse('1 month ago').moveToFirstDayOfMonth();  }, dateEnd: function(){ return Date.parse('1 month ago').moveToLastDayOfMonth();  } } /*The previous Month*/
    ],
    presets: {
      dateRange: trans('Выбор периода', 'Izvēlieties periodu',  'Date range')
    },
		rangeStartTitle: trans('Начало периода', 'Sākuma datums',  'Start date'),
		rangeEndTitle: trans('Конец периода', 'Beigu datums',  'End date'),
		nextLinkText: trans('Дальше', 'Nakamais',  'Next'),
		prevLinkText: trans('Назад', 'Iepriekšējais',  'Prev'),
		doneButtonText: trans('Подтвердить', 'Apstiprināt',  'Done'),
...
if (typeof(lng) != 'string' || lng == 'en')
  ...
  Date.CultureInfo={name:"en-US",englishName:"English (United States)",nativeName:"English (United States)",dayNames:["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],abbreviatedDayNames:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],shortestDayNames:["Вс","Пн","Вт","Ср","Чт","Пт","Сб"],firstLetterDayNames:["S","M","T","W","T","F","S"],monthNames:["Январь","Февраль","Март","Апрель","Май","Июнь","Июль","Август","Сентябрь","Октябрь","Ноябрь","Декабрь"],abbreviatedMonthNames:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],amDesignator:"AM",pmDesignator:"PM",firstDayOfWeek:1,twoDigitYearMax:2029,dateElementOrder:"mdy",formatPatterns:{shortDate:"M/d/yyyy",longDate:"dddd, MMMM dd, yyyy",shortTime:"h:mm tt",longTime:"h:mm:ss tt",fullDateTime:"dddd, MMMM dd, yyyy h:mm:ss tt",sortableDateTime:"yyyy-MM-ddTHH:mm:ss",universalSortableDateTime:"yyyy-MM-dd HH:mm:ssZ",rfc1123:"ddd, dd MMM yyyy HH:mm:ss GMT",monthDay:"MMMM dd",yearMonth:"MMMM, yyyy"},regexPatterns:{jan:/^jan(uary)?/i,feb:/^feb(ruary)?/i,mar:/^mar(ch)?/i,apr:/^apr(il)?/i,may:/^may/i,jun:/^jun(e)?/i,jul:/^jul(y)?/i,aug:/^aug(ust)?/i,sep:/^sep(t(ember)?)?/i,oct:/^oct(ober)?/i,nov:/^nov(ember)?/i,dec:/^dec(ember)?/i,sun:/^su(n(day)?)?/i,mon:/^mo(n(day)?)?/i,tue:/^tu(e(s(day)?)?)?/i,wed:/^we(d(nesday)?)?/i,thu:/^th(u(r(s(day)?)?)?)?/i,fri:/^fr(i(day)?)?/i,sat:/^sa(t(urday)?)?/i,future:/^next/i,past:/^last|past|prev(ious)?/i,add:/^(\+|after|from)/i,subtract:/^(\-|before|ago)/i,yesterday:/^yesterday/i,today:/^t(oday)?/i,tomorrow:/^tomorrow/i,now:/^n(ow)?/i,millisecond:/^ms|milli(second)?s?/i,second:/^sec(ond)?s?/i,minute:/^min(ute)?s?/i,hour:/^h(ou)?rs?/i,week:/^w(ee)?k/i,month:/^m(o(nth)?s?)?/i,day:/^d(ays?)?/i,year:/^y((ea)?rs?)?/i,shortMeridian:/^(a|p)/i,longMeridian:/^(a\.?m?\.?|p\.?m?\.?)/i,timezone:/^((e(s|d)t|c(s|d)t|m(s|d)t|p(s|d)t)|((gmt)?\s*(\+|\-)\s*\d\d\d\d?)|gmt)/i,ordinalSuffix:/^\s*(st|nd|rd|th)/i,timeContext:/^\s*(\:|a|p)/i},abbreviatedTimeZoneStandard:{GMT:"-000",EST:"-0400",CST:"-0500",MST:"-0600",PST:"-0700"},abbreviatedTimeZoneDST:{GMT:"-000",EDT:"-0500",CDT:"-0600",MDT:"-0700",PDT:"-0800"}};
}
